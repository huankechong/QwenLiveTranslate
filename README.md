...
| 悬浮字幕 | 透明圆角置顶窗，可拖动；显示/隐藏淡入淡出过渡；流式增量写入（高频更新零闪烁），Ctrl+Alt+Space 或控制台按钮切换点击穿透 |
...
- 原文行浅蓝、译文行白色大字；翻译历史点控制台「历史」查看/导出 Excel(.xlsx) 或 CSV
...
├── providers/          # 多引擎抽象层（LiveEngine 门面/显式注册表/通用推流循环；qwen 一体化包装，多模型支持铺路）
├── capture.py           # 音频采集（mic / loopback + 重采样；--list-devices 枚举设备）
├── overlay.py           # 悬浮双语字幕窗（连续速录+句数裁剪+位置记忆+延迟角标+点击穿透）
├── history.py           # 翻译历史 SQLite 存储（线程安全+CSV/xlsx 双格式导出）
